 ...
 *
 * This Google Apps Script lives INSIDE the Jobs Google Sheet (Extensions → Apps Script).
 *
 * Purpose (high level):
 * 1) Reviewers mark each Google Form submission as PENDING / APPROVE / REJECT in the Form_Responses tab.
 * 2) When a row is APPROVE:
 *    - Script appends a curated row into the "For Show" tab (the one that feeds jobs.json generation).
 *    - Script updates the Form_Responses Status to "APPROVED (MOVED)" (or duplicate note).
 * 3) When a row is REJECT:
 *    - Script sets Status to "REJECTED"
 *    - Script copies the row to a "Rejected" tab for recordkeeping.
 * 4) A daily trigger can email reviewers when there are pending submissions.
 *
 * Tabs expected:
 * - Form_Responses (Google Form responses)
 * - For Show (curated sheet that generates jobs.json)
 * - Rejected (optional; auto-created)
 *
 * IMPORTANT MAINTAINER NOTES
 * - This script depends on exact *header text* in row 1 for several columns.
 * - If you rename columns/tabs, update BOTH this script and the Python jobs.json generator.
 * - Prefer adding new columns to the far right, so existing column order stays stable.
 */
...
// Status values script writes (so reviewers know the row was processed)
...
function setupNYRGJobApproval() {
  ...
  // Apply validation down the column (we add extra buffer rows to cover future submissions)
  ...
  // NOTE: simple triggers can be limited; installable triggers are more reliable for editors.
...
function onEditNYRGJobApproval(e) {
  ...
  // Pull values by header name so column order can change (as long as header text stays the same).
  ...
  if (newValue === STATUS_APPROVE) {
    // Duplicate guard: avoids accidentally double-posting if someone approves twice.
    if (jobAlreadyExists_(jobs, title, company)) {
      form.getRange(row, statusCol).setValue(STATUS_MOVED + " (DUPLICATE FOUND)");
      return;
    }

    // Append to For Show tab.
    // Column mapping MUST match your jobs.json generator expectations.
    jobs.appendRow([
      title,
      company,
      desc,
...
function jobAlreadyExists_(jobsSheet, titleRaw, companyRaw) {
  ...
  // We only compare Title + Company (columns A and B).
